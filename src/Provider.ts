  ...
  provideDefinition(
    ...
    const reverse = (extensionsIndex: number): string | null => {
      const relativePath =
        extensionsIndex === -1
          ? word
          : `${word}${this.extensions[extensionsIndex]}`;
      ...
      try {
        ...
        if (state.isFile()) {
          ...
          if (
            ...
            [".js", ".jsx", ".ts", ".tsx"].includes(
              this.extensions[extensionsIndex]
            )
          ) {
            return null;
          }
          ...
          if (state.isDirectory()) {
            for (let i = 0; i < arr.length; i++) {
              const isExit = fs.existsSync(
                path.resolve(tempPath, `./index${arr[i]}`)
              );
              if (isExit) {
        ...
        // not exit tempPath
        if (extensionsIndex < this.extensions.length)
          return reverse(extensionsIndex + 1);
      }
      return null;
    };
